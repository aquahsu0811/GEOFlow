...
- **Method pattern**: Use the core job description as the anchor for comparison: Use when a user needs to run repeated DeepSeek web AI-search crawls for multiple questions, aggregate probabilistic brand/source/ranking evidence, and generate a JSON dataset plus a Kami-styled HTML analysis report. Not for generic web scraping, DeepSeek API chat, or one-off answer writing.
