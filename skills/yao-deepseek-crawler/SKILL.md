...
6. Stage 2: run `scripts/analyze_deepseek_results.py` on any crawl JSON with target entity/type, optional brands file, and report output dir.
7. Return the raw crawl JSON, structured Markdown, structured Excel workbook, HTML report, summary JSON, and failed logs. Reports include target-vs-best-3 radar, click-to-reveal bubbles, Chinese source names, clickable citations, title intent, compact treemap, and GEO actions.
...
- Inferred competitors are heuristic; review aliases and competitor tables before external use.
