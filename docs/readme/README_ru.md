...
- **Учётная запись по умолчанию:** production-сервис `init` запускает `db:seed` после миграций и создаёт только начальную admin-учётную запись; повторный запуск не перезаписывает существующего пользователя `admin`. Демо-данные фронтенда по умолчанию отключены и даже при явном включении не перезаписывают настройки сайта, рекламу, категории и статьи пользователя.
...
## Учётные данные по умолчанию (после `AdminUserSeeder`)
